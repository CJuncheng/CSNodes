...
#### 进程状态重置
execve函数**在当前进程的上下文加载并运行一个新程序**，如果把进程看作状态机，相当于**将当前运行的状态机重置成另一个程序的初始状态**。
```c
#include <unistd.h>
...
一个进程可以由多个**线程**组成，每个线程运行在进程的上下文，并共享**代码段、数据段、堆、共享库和打开的文件**，但有独立的的**TID、PC和寄存器**。每个线程对应一个**函数栈帧链**，在进程虚拟地址空间中，每个线程的栈为8M大小。

...
- **一致性（consistency）**：事务执行前后，数据从一个**合法性状态**变换到另外一个**合法性状态**。这种状态是**语义上**的而不是语法上的，跟具体的业务有关。满足**预定的约束**（世俗的约定）的状态就叫做合法的状态。
- **隔离性（isolation）**：指一个事务的执行**不能被其他事务干扰**，即一个事务内部的操作及使用的数据对**并发**的其他事务是隔离的，并发执行的各个事务之间不能互相干扰。比如 事物A, B分别表示A, B从同一个银行账户取钱，事物A, B不能同时发生，类似OS互斥。

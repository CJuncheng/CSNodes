...
6. ``git remote add origin https://github.com/CJuncheng/CodeCraft2022.git `` ： 添加新的git方式的origin, github上创建好的仓库和本地仓库进行关联 
7. ``git config --global http.sslVerify "false"`` : 连接超时执行
8. ``git push -u origin master`` : 把本地库的所有内容推送到远程仓库（github）上，即上传本地文件
